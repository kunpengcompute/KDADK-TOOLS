...
|特性名称|特性介绍|
|--|--|
| 网络流量分类 | 网络流量分类特性提供了特征提取、特征推理接口头文件及软件包，同时提供了鲲鹏BoostKit智能数据分析开发套件的网络流分类参考示例。 |
...
KDADK-TOOLS的环境部署安装方法可以参考《[安装指南](docs/zh/installation_guide.md)》。

编译得到可执行的二进制程序：`kdadk_demo`。

## 快速上手

KDADK-TOOLS的快速入门通过使用`kdadk_demo`来实现，具体请参见《[快速入门](docs/zh/quick_start.md)》。
...
- 本项目仅提供参考示例及工具，使用者需自行承担使用风险，并理解以下内容：
    ...
    - 用户输入安全性：用户需自行保证输入的命令的安全性，并承担因输入不当而导致的任何安全风险或损失。对于输入命令不当所导致的问题，本工具及其开发者概不负责。
...
本项目的文档采用CC-BY 4.0许可证，具体请参见[LICENSE](docs/LICENSE)文件。
