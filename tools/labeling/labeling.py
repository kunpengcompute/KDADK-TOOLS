...
def load_and_concat(folder_path: str, inference=False):
    """
    ...
    if not all_csv_files:
        logging.warning(f"警告: 在 {folder_path} 中未找到CSV文件")
        return pd.DataFrame()  # 返回空DataFrame
    
    dfs = load_data(all_csv_files, inference)
    return pd.concat(dfs, axis=0, ignore_index=True)

def load_and_prepare_data(data_paths: list, labels: list = None, inference=False):
    """
    加载并准备数据，为多个数据源添加标签
    ...
    for idx, path_info in enumerate(normalized_paths):
        ...
        logging.info(f"[{idx + 1}/{len(normalized_paths)}] 加载 {name} 数据...")
        ...
        try:
            ...
            if not data.empty:
                ...
                logging.info(f"{name} 数据: {len(data)} 条记录")
            else:
                logging.warning(f"警告: {name} 数据集为空，跳过")

        except Exception as e:
            logging.error(f"加载 {name} 数据时出错: {str(e)}")
            continue
    ...
    if 'raw_bow' in X.columns:
        ...
        logging.info("已提取 raw_bow 列")
    else:
        logging.info("未找到 raw_bow 列")
    
    y = combined_data['label'].values  # 转换为numpy数组
    
    # 打印统计信息
    logging.info("数据集统计信息")
    logging.info(f"总数据量: {len(combined_data)}")
...
def print_evaluation_report(best_k, train_silhouette, test_silhouette, inertia, 
                          n_train, n_test, results_dir):
    """
    打印评估报告
    """
    logging.info("K-Means 模型评估报告")
    logging.info(f"· 使用的聚类数 k: {best_k}")
    logging.info(f"· 训练集轮廓系数: {train_silhouette:.4f}")
    logging.info(f"· 测试集轮廓系数: {test_silhouette:.4f}")
    logging.info(f"· 训练集惯性: {inertia:.4f}")
    logging.info(f"· 训练集样本数: {n_train}")
    logging.info(f"· 测试集样本数: {n_test}")
    logging.info(f"· 结果保存路径: {results_dir}/")
...
def train_model(X, n_clusters=6, test_size=0.3, random_state=42, model_name='kmeans', results_dir="results"):
    ...
    logging.info(f"训练集数据：{X_train.shape[0]}, 测试集：{X_test.shape[0]}")

    # 3. 验证指定的聚类数
    max_clusters = min(len(X_train)//2, len(X_train))
    if n_clusters > max_clusters:
        logging.warning(f"警告：指定的聚类数 {n_clusters} 过大，调整为 {max_clusters}")
        n_clusters = max_clusters
    elif n_clusters < 2:
        logging.warning(f"警告：指定的聚类数 {n_clusters} 过小，调整为 2")
        n_clusters = 2
    
    logging.info(f"使用指定的聚类数 k = {n_clusters}")

    logging.info(f"训练模型 (k={n_clusters})...")
    # 统计训练时间
    ...
    end_time = time.time()
    logging.info(f"模型训练完成，耗时 {end_time - start_time:.2f} 秒")

    joblib.dump(kmeans, f"{results_dir}/{model_name}_model.pkl")
    joblib.dump(scaler, f"{results_dir}/{model_name}_scaler.pkl")
    logging.info(f"模型已保存到 {results_dir}/{model_name}_model.pkl")
    logging.info(f"标准化器已保存到 {results_dir}/{model_name}_scaler.pkl")
...
def save_clustering_results(input_data: pd.DataFrame, 
    ...
    if raw_bow is not None:
        ...
        logging.info(f"已添加raw_bow列，包含 {len(raw_bow_processed)} 个条目")
        
        # 统计raw_bow列的数据情况
        non_empty_count = sum(1 for x in raw_bow_processed if str(x).strip() != '')
        empty_count = len(raw_bow_processed) - non_empty_count
        logging.info(f"raw_bow列统计: 非空数据 {non_empty_count} 条，空数据 {empty_count} 条")
    else:
        logging.warning("警告: raw_bow数据为空，将创建空的raw_bow列")
        result_df['raw_bow'] = ''
    ...
    # result_df.to_csv(output_path, index=False)
    logging.info(f"聚类结果已保存至: {output_path}")
    logging.info(f"输出文件包含 {len(result_df)} 行数据，{len(result_df.columns)} 列特征")
...
def training(config):
    ...
    try:
        data_paths = config["train.data_paths"]
        model_name = config['train.model_name']
        n_clusters = config.get('train.n_clusters')
        test_size = config.get('train.test_size')
        results_dir = config.get('train.results_dir')

        if not data_paths or not isinstance(data_paths, list) or len(data_paths) == 0:
            raise ValueError("配置错误: 'train.data_paths' 必须是非空列表")

        if not isinstance(model_name, str) or not model_name.strip():
            raise ValueError("配置错误: 'train.model_name' 必须是非空字符串")

        if not isinstance(n_clusters, int) or n_clusters <= 0:
            raise ValueError(f"配置错误: 'train.n_clusters' 必须是正整数")

        if not isinstance(test_size, (int, float)) or not (0 < test_size < 1):
            raise ValueError(f"配置错误: 'train.test_size' 必须在 (0, 1) 范围内")

        if not isinstance(results_dir, str) or not results_dir.strip():
            raise ValueError("配置错误: 'train.results_dir' 必须是非空字符串")
        # 1. 加载并准备数据
...
def inferenceing(config):
    ...
    try:
        if not data_paths or not isinstance(data_paths, list) or len(data_paths) == 0:
            raise ValueError("'inference.data_paths' 必须是非空列表")
        ...
        if not scaler_path or not isinstance(scaler_path, str) or not os.path.exists(scaler_path):
        ...
        logging.info(f"聚类结果统计:")
        for cluster, count in zip(unique_clusters, counts):
            percentage = count/len(cluster_preds)*100
            logging.info(f"聚类 {cluster}: {count} 个样本 ({percentage:.2f}%)")
